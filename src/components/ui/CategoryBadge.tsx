...
export const CategoryBadge: React.FC<CategoryBadgeProps> = ({ children, className = '' }) => {
    return (
        <span className={`inline-flex items-center px-2 py-0.5 rounded-md border border-gray-200 bg-gray-50 text-xs2 font-semibold text-gray-600 uppercase tracking-widest dark:bg-slate-800 dark:border-slate-700 dark:text-gray-400 ${className}`}>
            {children}
        </span>
    );
};
