...
import { XMarkIcon, Cog6ToothIcon, DocumentTextIcon, CheckCircleIcon, ExclamationTriangleIcon } from '../../../components/ui/Icons';
import { useUI } from '../../../context/UIContext';
import { ArqueoRecord } from '../../../types';
import { KlaveConfigModal } from './KlaveConfigModal';
import { formatDateToDisplay } from '../../../utils/dateUtils';
import { DatePicker } from '../../../components/ui/DatePicker';
import { FormGroup } from '../../../components/ui/FormGroup';

interface KlaveExportWizardProps {
    isOpen: boolean;
    onClose: () => void;
...
export const KlaveExportWizard: React.FC<KlaveExportWizardProps> = ({ isOpen, onClose, selectedArqueos = [] }) => {
            ...
            config.mappings.forEach(map => {
                ...
                if (amount > 0) {
                    ...
                    entries.push({
                        ...
                        credito: credit,
                        base: 0, 
                        descripcion: `Cierre Caja ${arqueoDate}`,
                        descripcionMovimiento: map.label
                    });
                }
            });
    ...
    const headerTitle = (
        <span className="flex items-center gap-2">
            ...
            <span className="text-lg font-bold">Exportar Klave</span>
        </span>
    );
                    ...
                    <div className="min-h-[200px]">
                            ...
                            <div className="text-center py-8 space-y-4">
                                {!hasValidConfig ? (
                                    <div className="p-4 bg-amber-50 dark:bg-amber-900/20 text-amber-800 dark:text-amber-200 rounded-lg border border-amber-200">
                                        ...
                                        <p className="text-sm">No se ha detectado una configuración para Klave. Debes configurarla antes de continuar.</p>
                                    </div>
                                ) : (
                                    <div className="p-4 bg-green-50 dark:bg-green-900/20 text-green-800 dark:text-green-200 rounded-lg border border-green-200">
                                        ...
                                        <p className="text-sm">Se encontraron {config?.mappings?.length || 0} cuentas configuradas para Klave.</p>
                                    </div>
                                )}
                        ...
                        {step === 2 && (
                            <div className="space-y-6 max-w-sm mx-auto">
                                ...
                                <FormGroup label="Consecutivo Inicial" description="Se incrementará automáticamente para cada arqueo.">
                                    <input
                                        type="number"
                                        value={consecutive}
